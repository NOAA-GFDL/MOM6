...
!! such location on the unrotated version of the grid within that layer.
  subroutine array_global_min_max(tr_array, G, nk, g_min, g_max, &
    ...
    type(ocean_grid_type),        intent(in)   :: G      !< The ocean's grid structure
    real, dimension(SZI_(G),SZJ_(G),nk), intent(in)  :: tr_array !< The tracer array to search for extrema [CU ~> conc]
    real,                         intent(out) :: g_min !< The global minimum of tr_array [conc]
    real,                         intent(out) :: g_max !< The global maximum of tr_array [conc]
    ...
    ! Local variables
    real    :: tmax, tmin   ! Maximum and minimum tracer values, in the same units as tr_array [conc]
    real    :: unsc         ! A factor to use to undo any scaling of the input tracer array [conc CU-1 ~> 1]
    integer :: ijk_min_max(2)  ! Integers encoding the global grid positions of the minimum and maximum values
    ...
    unsc = 1.0 ; if (present(unscale)) unsc = unscale

    ! Find the maximum and minimum tracer values on this PE and their locations.
    tmax = -1.e34 ; tmin = 1.e34
    do k=1,nk ; do j=jsc,jec ; do i=isc,iec ; if (G%mask2dT(i,j) > 0.0) then
      valid_PE = .true.
      if (unsc*tr_array(i,j,k) > tmax) then
        tmax = unsc*tr_array(i,j,k)
        itmax = i ; jtmax = j ; ktmax = k
      elseif ((unsc*tr_array(i,j,k) == tmax) .and. (k <= ktmax)) then
        if (ijk_loc(i, j, k, nk, G%HI) > ijk_loc(itmax, jtmax, ktmax, nk, G%HI)) then
          itmax = i ; jtmax = j ; ktmax = k
        endif
      endif
      if (unsc*tr_array(i,j,k) < tmin) then
        tmin = unsc*tr_array(i,j,k)
        itmin = i ; jtmin = j ; ktmin = k
      elseif ((unsc*tr_array(i,j,k) == tmin) .and. (k <= ktmin)) then
        if (ijk_loc(i, j, k, nk, G%HI) > ijk_loc(itmin, jtmin, ktmin, nk, G%HI)) then
          itmin = i ; jtmin = j ; ktmin = k
        endif
      endif
    endif ; enddo ; enddo ; enddo

    if (.not.(present(xgmin) .or. present(ygmin) .or. present(zgmin) .or. &
              present(xgmax) .or. present(ygmax) .or. present(zgmax))) return
    ...
    if (valid_PE) then
      if (g_min == tmin) then
        ijk_min_max(1) = ijk_loc(itmin, jtmin, ktmin, nk, G%HI)
      endif
      if (g_max == tmax) then
        ijk_min_max(2) = ijk_loc(itmax, jtmax, ktmax, nk, G%HI)
      endif
    endif
    ! If MOM6 supported taking maxima on arrrays, these could be combined as:
    ...
    xyz_min_max(1:6) = -1.0e35  ! This huge negative value should never be used in the end.
    if (valid_PE) then
      if (ijk_min_max(1) == ijk_loc(itmin, jtmin, ktmin, nk, G%HI)) then
        xyz_min_max(1) = G%geoLonT(itmin,jtmin)
        xyz_min_max(2) = G%geoLatT(itmin,jtmin)
        xyz_min_max(3) = real(ktmin)
      endif
      if (ijk_min_max(2) == ijk_loc(itmax, jtmax, ktmax, nk, G%HI)) then
  ...
  function ijk_loc(i, j, k, nk, HI)
    ...
    ! Local variables
    integer :: ig, jg  ! Global index values with a global computationl domain start value of 1.
    integer :: ij_loc  ! The encoding of the horizonal position
