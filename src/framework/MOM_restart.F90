...
logical function size_mismatch_3d(var_a, var_b, turns, size_msg)
  ...
  write(size_msg, '(3(x,I0), " vs ", 3(x,I0))') size(var_a,1), size(var_a,2), size(var_a,3), &
...
subroutine save_restart(directory, time, G, CS, time_stamped, filename, GV, num_rest_files, write_IC)
  ...
  do while (next_var <= CS%novars )
    ...
    do m=start_var,next_var-1
      ...
      if (verbose) then
        if (pos == CENTER) then
          write(mesg, '(" is in CENTER position, checksum range",4(x,I0))') isL, ieL, jsL, jeL
        elseif (pos == CORNER) then
          write(mesg, '(" is in CORNER position, checksum range",4(x,I0))') isL, ieL, jsL, jeL
        elseif (pos == NORTH_FACE) then
          write(mesg, '(" is in NORTH_FACE position, checksum range",4(x,I0))') isL, ieL, jsL, jeL
        elseif (pos == EAST_FACE) then
          write(mesg, '(" is in EAST_FACE position, checksum range",4(x,I0))') isL, ieL, jsL, jeL
        else
          write(mesg, '(" is in another position, ",I0,", checksum range",4(x,I0))') pos, isL, ieL, jsL, jeL
        endif
