...
subroutine vertvisc_coef(u, v, h, dz, forces, visc, tv, dt, G, GV, US, CS, OBC, VarMix)
  ...
  if (associated(OBC)) then
    ...
    js_N_OBC = max(js, OBC%js_v_N_obc) ; je_N_OBC = min(je, OBC%je_v_N_obc)
    js_S_OBC = max(js, OBC%js_v_S_obc) ; je_S_OBC = min(je, OBC%je_v_S_obc)
    ...
    Js_E_OBC = max(Jsq, OBC%Js_u_E_obc) ; Je_E_OBC = min(Jeq, OBC%Je_u_E_obc)
    Js_W_OBC = max(Jsq, OBC%Js_u_W_obc) ; Je_W_OBC = min(Jeq, OBC%Je_u_W_obc)
  endif
  ...
  if (associated(OBC)) then
    ...
    if (OBC%u_W_OBCs_on_PE) then
      do j=js_W_OBC,je_W_OBC ; do I=Is_W_OBC,Is_E_OBC
        if (do_i(I,j) .and. OBC%segnum_u(I,j) < 0) then
  ...
  ! FIXME: These can be moved to the diagnostic stage
