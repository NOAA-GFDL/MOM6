...
subroutine calc_sqg_struct(h, tv, G, GV, US, CS, dt, MEKE)
  ...
  call calc_isoneutral_slopes(G, GV, US, h, e, tv, dt*CS%kappa_smooth, CS%use_stanley_iso, &
                                  ...
                                  dzSxN=dzSxN, dzSyN=dzSyN, halo=1)  !### Add , OBC=OBC, OBC_N2=OBC_N2=CS%OBC_friendly)
  ...
  if (associated(OBC).and. CS%OBC_friendly) then
   ...
   !$OMP parallel do default(shared)
    ...
    do K=2,nz
      ...
      enddo ; enddo
    enddo
  else  ! The land mask is sufficent and there are no special considerations taken at OBC points.
